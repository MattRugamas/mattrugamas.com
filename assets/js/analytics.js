(function () {
  ...
  // ─── 6. About page music CTA ("See what I've been playing →") ────────────
  var musicCta = document.querySelector('.music-link a');
  if (musicCta) {
  ...
  // ─── 7. Résumé print/save button ─────────────────────────────────────────
  var printBtn = document.getElementById('resume-print-btn');
  if (printBtn) {
    printBtn.addEventListener('click', function () {
      track('resume_print_click', {
        page_path: window.location.pathname
      });
    });
  }

  // ─── 8. Résumé LinkedIn click ────────────────────────────────────────────
  var cvContact = document.querySelector('.cv-header-contact a');
  if (cvContact) {
    cvContact.addEventListener('click', function () {
      track('resume_linkedin_click', {
        link_url: this.getAttribute('href')
      });
    });
  }
  ...
  // ─── 10. Post footer link clicks ─────────────────────────────────────────
  var postFooter = document.querySelector('.post-footer');
  if (postFooter) {
    postFooter.querySelectorAll('a').forEach(function (link) {
      link.addEventListener('click', function () {
  ...
  // ─── 12. Link hub (/links/) outbound clicks ───────────────────────────────
  var linkHub = document.querySelector('.link-hub');
  if (linkHub) {
    linkHub.querySelectorAll('.link-hub-list a').forEach(function (link) {
      link.addEventListener('click', function () {
  ...
  function checkScrollDepth() {
    ...
    depthMarkers.forEach(function (marker) {
      if (!depthsFired[marker] && pct >= marker) {
  ...
  // Catch pages that are already scrolled on load (e.g. hash anchors)
  checkScrollDepth();
